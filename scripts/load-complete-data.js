...
// Parse stations CSV
function parseStationCSVLine(line) {
...
// Parse trains CSV
function parseTrainCSVLine(line) {
    const parts = line.split(',');
    if (parts.length < 14)
        return null;
    return {
...
function determineOperatingDays() {
    return 'DAILY';
}
function generateStationCode(name, id) {
...
async function loadCompleteData() {
    try {
        ...
        circumvesuvianaStations.sort((a, b) => {
            ...
            if (a.name === 'Napoli Porta Nolana')
                return -1;
            if (b.name === 'Napoli Porta Nolana')
                return 1;
        ...
        for (const line of trainLines) {
            ...
            try {
                ...
                const activeStations = routeStations.filter(s => s.isStation === true &&
                    s.isDismessa === false &&
                    stationMap.has(s.id));
